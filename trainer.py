...
class EarlyStopping:
    ...
    Guarda solo el state_dict en lugar de deepcopy del módulo.
    """
    ...
    def __call__(self, model: torch.nn.Module, val_loss: float) -> bool:
        ...
        if self.best_loss is None:
            self.best_loss = val_loss
            # Guardamos una copia del state_dict en CPU
            self.best_state = {k:v.cpu().clone() for k,v in model.state_dict().items()}
            self.status = f"{self.counter}/{self.patience}"
            return False

        # Si mejora de forma significativa
        ...
        # En cualquier otro caso (igual o peor) incrementamos
        self.counter += 1
        if self.counter >= self.patience:
            self.status = f"Stopped on {self.counter}"
            if self.restore_best_weights and self.best_state is not None:
                # Recargamos el mejor state_dict
